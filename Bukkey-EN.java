...
public class Bukkey {
    ...
    // *-- You can change the prefix, but please don't modify any other fields! --*
    ...
    public Bukkey(Plugin plugin, String licenseKey, String pluginId, boolean isEnabled) {
        ...
        if (!isEnabled) {
            plugin.getLogger().info(prefix + "§e License key verification is disabled.");
            return;
        }

        this.ip = getPublicIP();
        if ("ERROR".equals(ip)) {
            plugin.getLogger().warning(prefix + "§c Failed to retrieve your public IP address. Verification impossible.");
        ...
        plugin.getLogger().info("§e│§r §fYour fingerprint:§e " + printFingerprint());
        plugin.getLogger().info("§e│§r §fStatus:§e Checking license key...");
        plugin.getLogger().info("§e┕");
        plugin.getLogger().info(" ");
    ...
    private void checkLicenseKey() {
        ...
        try {
            ...
            if (response.contains("\"success\":true")) {
                plugin.getLogger().info(prefix + " §aLicense verified successfully! Plugin activated.");
            } else {
                String errorMessage = extractJsonField(response, "error");
                if (errorMessage == null || errorMessage.isEmpty()) {
                    errorMessage = "Unknown error (code " + statusCode + ")";
                }
                plugin.getLogger().warning(prefix + "§c License error: " + errorMessage);
            ...
            plugin.getLogger().severe(prefix + "§c Error during license check: " + e.getMessage());
            Bukkit.getScheduler().runTask(plugin, () ->
                    Bukkit.getPluginManager().disablePlugin(plugin)
            );
        } finally {
